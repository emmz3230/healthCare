...
const AiResponse: React.FC<AiResponseProps> = ({ HealthStatus, isLoading }) => {
    ...
    <div className='h-[85vh] w-[75%] bg-[#2893DF] flex items-center justify-center'>
      <div className='h-[90%] w-[90%] bg-white shadow-lg rounded-lg flex flex-col'>
        ...
        <div className='flex-1 overflow-auto'>
          {index === 1 ? <Diet HealthStatus={HealthStatus} isLoading={isLoading} /> :
           index === 2 ? <Exercise /> :
           index === 3 ? <Activities /> :
           <Diet HealthStatus={HealthStatus} isLoading={isLoading} />}
        </div>
      </div>
    </div>
  );
